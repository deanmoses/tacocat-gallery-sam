...
const mediaExtPattern = [...IMAGE_EXTENSIONS, ...VIDEO_EXTENSIONS].join('|');
const pathMediaIdPattern = new RegExp(
    `^\\/i\\/(?<ID>\\d\\d\\d\\d\\/(0[1-9]|1[0-2])-(0[1-9]|[12]\\d|3[01])\\/[a-zA-Z0-9_-]+\\.(${mediaExtPattern}))\\/(?<VERSION>[^/]+)`,
    'i',
);
