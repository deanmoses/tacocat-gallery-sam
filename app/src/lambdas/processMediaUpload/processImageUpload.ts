...
export async function processImageUpload(bucket: string, key: string, versionId: string | undefined): Promise<void> {
    const imagePath = '/' + key;
    console.info({ event: 'image_processing_started', imagePath });
    ...
    if ('jpg' !== imagePath.split('.').pop()?.toLowerCase()) {
        console.warn({ event: 'image_not_jpg', imagePath });
    }
    ...
    if (imageCreateRequest.dimensions) {
        ...
        console.error({ event: 'image_no_dimensions', imagePath });
    }

    console.info({ event: 'image_processing_complete', imagePath });
}
...
async function handleMetadataExtractionError(
    ...
    console.error({ event: 'metadata_extraction_failed', key, error: errorMessage });
