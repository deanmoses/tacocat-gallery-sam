...
describe('upsertVideo validation', () => {
    test('fail on invalid videoPath', async () => {
        await expect(upsertVideo('/invalid_path', videoId, versionId, dimensions, duration)).rejects.toThrow(
            /invalid.*path/i,
        );
    });

    test('fail on image path (not video)', async () => {
        await expect(upsertVideo('/2001/12-31/image.jpg', videoId, versionId, dimensions, duration)).rejects.toThrow(
            /invalid.*path/i,
        );
    });

    test('fail on missing videoId', async () => {
        await expect(upsertVideo(videoPath, '', versionId, dimensions, duration)).rejects.toThrow(/videoId/i);
    });

    test('fail on missing versionId', async () => {
        await expect(upsertVideo(videoPath, videoId, '', dimensions, duration)).rejects.toThrow(/versionId/i);
    });
});

describe('upsertVideo DynamoDB write', () => {
    test('writes correct fields to DynamoDB', async () => {
        await expect(upsertVideo(videoPath, videoId, versionId, dimensions, duration)).resolves.not.toThrow();
        ...
        expect(updateInput.ExpressionAttributeValues?.[':id']).toEqual(videoId);
    ...
    test('accepts .mov video path', async () => {
        await expect(
            upsertVideo('/2001/12-31/video.mov', videoId, versionId, dimensions, duration),
        ).resolves.not.toThrow();
    });

    test('accepts .webm video path', async () => {
        await expect(
            upsertVideo('/2001/12-31/video.webm', videoId, versionId, dimensions, duration),
        ).resolves.not.toThrow();
    });
});
