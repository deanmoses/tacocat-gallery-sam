...
function request(overrides: Partial<Request> = {}): Request {
    return { method: 'GET', uri: '/album/2001/12-31', querystring: {}, headers: {}, cookies: {}, ...overrides };
}
...
beforeEach(() => {
    ...
    store.set('nav:/', 'rootnav1');
    store.set('content:/2001/12-31/', 'day1');
    store.set('nav:/2001/', 'yearnav1');
});

test('keys a day album on its content version and its parent’s nav version', async () => {
    const r = await run();
    expect(r.headers['x-album-version']).toEqual({ value: 'day1.yearnav1' });
    expect(r.headers['x-has-token']).toEqual({ value: '0' });
});

test('keys a year album on the root’s nav version, and the root on content alone', async () => {
    expect((await run({ uri: '/album/2001' })).headers['x-album-version']).toEqual({ value: 'year1.rootnav1' });
    expect((await run({ uri: '/album' })).headers['x-album-version']).toEqual({ value: 'root1' });
});

test('an album missing from the store, or whose parent nav is missing, gets no version', async () => {
    store.delete('content:/2001/12-31/');
    expect((await run()).headers['x-album-version']).toBeUndefined();
    store.set('content:/2001/12-31/', 'day1');
    store.delete('nav:/2001/');
    expect((await run()).headers['x-album-version']).toBeUndefined();
});
...
test('?fresh gets a version nothing else will ask for and the query string is dropped', async () => {
...
test('writes and non-album paths pass through untouched', async () => {
    ...
    const junk = await run({ uri: '/album/2001/12-31/' });
    expect(junk.headers).toEqual({});
    const other = await run({ uri: '/album-thumb/2001/12-31' });
