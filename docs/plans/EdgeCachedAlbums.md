...
- If `UpdateAlbumVersions` drops a batch (it retries three times, then the errors show in its log stream), the affected versions stay stale until the cache entry's day-long `s-maxage` runs out and the next request triggers a background refresh, or `SyncAlbumVersions` is run. An alarm on its errors would be the right thing but the account is at the free tier's ten alarm metrics.
- The edge cache's TTL is a day, with a month of `stale-while-revalidate` and `stale-if-error` after it. Most albums see few requests per day per edge location, so a shorter TTL would mean most requests miss; the version in the key, not the TTL, is what keeps entries current. The stale window means an expired entry is served at once and refreshed behind the visitor, and still serves if the origin is down. Browsers implement `stale-while-revalidate` too, against the `max-age=0`, which only matters once the gallery app stops sending `no-store`.
