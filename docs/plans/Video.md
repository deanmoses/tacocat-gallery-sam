...
### UUIDs and Version-Aware Storage for Video Transcode & Poster
...
- **Simplifies rename** - Renaming a video only requires updating DynamoDB and copying the original. The transcoded video and poster stay put.
  - Right now during a rename we _delete_ the derived files, but I don't want to resubmit a job to AWS MediaConvert to re-generate the transcoded video and poster image, so we're touching rename _SOMEHOW_ here.
...
- **Immutable caching** - Each version can be cached forever (immutable). No cache invalidation needed on re-upload.
...
**Future consideration:** In the future, I would like to give still images a UUID too, and update the derived image generator to _always_ use UUID, thus avoiding having to delete the derived files on rename. See [DerivedMedia.md](./DerivedMedia.md) for the full plan.
