...
- **CI workflow**: On PR and push to main, runs all lint and unit test checks plus a SAM build (see `.github/workflows/`). On PR, also deploys the test stack and runs the integration tests against it; merging is gated on all of it. On push to main, deploys to staging.
